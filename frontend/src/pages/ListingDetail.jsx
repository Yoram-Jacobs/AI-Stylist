      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
        ...
        <div className="md:col-span-2 space-y-4">
            ...
            <CardContent className="p-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground mt-1"><Eye className="inline h-3 w-3 mr-1" />{listing.views || 0} views</div>
                </div>
          ...
          <Card className="rounded-[calc(var(--radius)+6px)] shadow-editorial" data-testid="listing-detail-fee-breakdown">
            <CardContent className="p-5">
              <div className="caps-label text-muted-foreground">Fee breakdown</div>
              <dl className="mt-3 text-sm space-y-2">
                <div className="flex justify-between"><dt className="text-muted-foreground">List price</dt><dd>{fmt(fm.list_price_cents, fm.currency)}</dd></div>
                <div className="flex justify-between"><dt className="text-muted-foreground">Stripe fee</dt><dd>− {fmt(fm.stripe_processing_fee_fixed_cents)} + 2.9%</dd></div>
                <div className="flex justify-between"><dt className="text-muted-foreground">Platform (7% after Stripe)</dt><dd></dd></div>
                <div className="flex justify-between font-medium border-t border-border pt-2"><dt>Seller receives</dt><dd>{fmt(fm.estimated_seller_net_cents, fm.currency)}</dd></div>
              </dl>
            </CardContent>
          </Card>
          ...
          {isOwner ? (
            <Button asChild variant="secondary" className="w-full rounded-xl" data-testid="listing-edit-button">
              <Link to={`/market`}>Manage in “My listings”</Link>
            </Button>
          ) : listing.status === 'active' ? (
            <Button onClick={onBuy} disabled={buying} className="w-full rounded-xl" data-testid="listing-buy-button">
              {buying ? <Loader2 className="h-4 w-4 animate-spin" /> : `Reserve for ${fmt(fm.list_price_cents, fm.currency)}`}
            </Button>
          ) : (
